...
RSpec.describe GalterIrExporter::Export::Actor do
  ...
  describe "register_convertor" do
    ...
    before do
      ...
      class TestConverter < GalterIrExporter::Export::Converter; end
    end
    ...
    context "invalid model" do
      ...
      it "throws an error" do
        expect { actor.register_converter(model_class, converter_class) }.to raise_error(GalterIrExporter::Export::RegistryError)
    ...
    context "default values" do
      it { is_expected.to eq(GenericFile => GalterIrExporter::Export::GenericFileConverter, Collection => GalterIrExporter::Export::CollectionConverter) }
    end
  end
  ...
  describe "call" do
    ...
    let(:options) { {} }
    let(:generic_file_converter) { GalterIrExporter::Export::GenericFileConverter.new(file) }
    let(:generic_file_file_name) { "tmp/export/generic_file_#{file.id}.json" }
    let(:collection_converter) { GalterIrExporter::Export::CollectionConverter.new(collection) }
    ...
    it "converts files and collections by default" do
      expect(GalterIrExporter::Export::GenericFileConverter).to receive(:new).and_return(generic_file_converter)
      expect(GalterIrExporter::Export::CollectionConverter).to receive(:new).and_return(collection_converter)
    ...
    it "validates the class list" do
      expect { actor.call([GenericFile, Collection, String]) }.to raise_error(GalterIrExporter::Export::RegistryError)
    end
