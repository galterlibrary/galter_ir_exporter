...
describe GalterIrExporter::Migration::Survey::Surveyor, type: :model do
  let(:collection) do
  ...
  describe "#call" do
    let(:ids) { [object.id] }
    subject { GalterIrExporter::Migration::Survey::Item.where(object_id: object.id).first }
    ...
    context "when multiple types of objects are passed" do
      ...
      let(:file_survey) { GalterIrExporter::Migration::Survey::Item.where(object_id: file.id).first }
      let(:collection_survey) { GalterIrExporter::Migration::Survey::Item.where(object_id: collection.id).first }
    ...
    context "when item is surveyed twice" do
      ...
      it "creates a single item" do
        expect(GalterIrExporter::Migration::Survey::Item.where(object_id: object.id).count).to eq 1
      end
    end
  end
  ...
  context "when one of the objects does not exist" do
    ...
    it "Errors with out creating any survey items" do
      ...
      expect(GalterIrExporter::Migration::Survey::Item.find_by(object_id: file.id)).to be_nil
    end
  end
end
